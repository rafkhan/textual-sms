...
public class ConversationListProvider extends ContentProvider {
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		String query = "INSERT INTO " + ConversationList.TABLE_NAME + "("
				+ ConversationList.COL_SENDER + ","
				+ ConversationList.COL_MESSAGE + ","
				+ ConversationList.COL_TIMESTAMP + ") VALUES ("
				+ "\'" + sender +"\',"
				+ "\'" + message +"\',"
				+ timestamp + ")";
				*/
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		Cursor result = null;
		if (URI_CONVERSATIONS.equals(uri)) {
			result = DatabaseHandler
					.getInstance(getContext())
					.getReadableDatabase()
					.query(ConversationList.TABLE_NAME,
							ConversationList.FIELDS, null, null, null, null,
							null, null);
			result.setNotificationUri(getContext().getContentResolver(),
					URI_CONVERSATIONS);
		} else if (uri.toString().startsWith(CONVERSATIONS_BASE)) {
